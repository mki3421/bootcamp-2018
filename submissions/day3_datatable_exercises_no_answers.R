...
select(generation_df, small_hydro) #with dplyr
...
generation_dt[,.(datetime, small_hydro)] #.() is same as list()
...
generation_dt[,all_hydro := small_hydro + large_hydro] #makes new column to generation_dt. following string does the same thing
...
total_renew <- generation_dt[,.(datetime,solar, wind, htotal_re = solar + wind)]
#total_renew[,dtotal_re := sum(htotal_re), by = day(datetime)]
total_renew[,sum(htotal_re), by = day(datetime)]
